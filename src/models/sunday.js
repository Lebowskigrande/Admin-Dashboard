...
const normalizeService = (service, dayDate, peopleList = PEOPLE) => {
    const roster = {};
    ROLE_KEYS.forEach((roleKey) => {
        const valueFromRolesObject = service?.roles?.[roleKey];
        const valueFromLegacyKey = service?.[roleKey];
        roster[roleKey] = normalizeAssignments(valueFromRolesObject || valueFromLegacyKey, roleKey, peopleList);
    });
...
export const createSundayFromApiDay = (day, peopleList = PEOPLE) => {
    ...
    return {
        ...
        services: Array.isArray(day.services)
            ? day.services.map(service => normalizeService(service, dayDate, peopleList))
            : [],
